...
const Login = ({ route, navigation }) => {
	...
	const onPressLoginproducao = () => {
		navigation.navigate('Loginproducao', { from: 'loginproducao' });
	};
	...
	const handleSubmit = async () => {
		if (validateInputs()) {
			try {
				...
				if (response.data.user.tipo === 'promotor') {
	...
	return (
		<KeyboardAvoidingView enabled={Platform.OS === 'ios' || Platform.OS === 'android'} behavior="padding" style={styles.main}>
			...
			<View style={styles.textlogin}>
				<FontAwesomeIcon icon={faTruck} size={30} style={styles.iconstbn} />
				<Text style={styles.textfont}>Logue-se como promotor</Text>
			</View>
			...
			<View style={styles.buttons}>
				<TouchableOpacity style={styles.buttonFirst} onPress={onPressLogin}>
					<Text style={styles.buttonsText}>Logar como promotor</Text>
				</TouchableOpacity>
			</View>
			...
			<View style={styles.buttons2}>
				<TouchableOpacity style={styles.buttonSecound} onPress={onPressLoginproducao}>
					<FontAwesomeIcon icon={faUser} size={30} style={styles.iconstbn} />
					<Text style={styles.buttonsText}>Logue-se como produção</Text>
				</TouchableOpacity>
			</View>
			{/*
