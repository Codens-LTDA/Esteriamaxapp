...
const Deliveredreport = ({ client, departament, date }) => {
    ...
    React.useEffect(() => {
        async function date() {
            try {
                ...
                setLoading(true)
                const response = await api.get('/reportdelivered', {
                ...
                console.log(response.data.delivered)
    ...
    return (
        <View>
            <View style={styles.interntclient}>
                <Text style={styles.textclients}>Caixa(s) entregue(s) neste cliente: </Text>
                {
                    loading === false && delivereds.map((delivered, index) => (
                        <View style={styles.delivered}>
                            <View style={styles.numberbox}>
                                <Text>{delivered.caixa}</Text>
                            </View>
                            <View>
                                <Text>{delivered.cnt_name}</Text>
                            </View>
                        </View>
                    ))
                }
                {
                    loading === false && delivereds.length === 0 ?
                        (
                            <View style={styles.none}>
                                <Text style={styles.nonetext}>
                                    Nenhuma caixa entregue foi encontrada.
                                </Text>
                            </View>
                        )
                        :
